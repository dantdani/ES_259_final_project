...
class PlainMLP(nn.Module):
    """Plain Multi-Layer Perceptron (no residuals).
    
    Architecture:
        Linear(input_dim -> hidden_dim) -> GELU
        -> (num_hidden_layers - 1) x [Linear(hidden_dim -> hidden_dim) -> GELU -> Dropout]
...
def build_model(arch="resmlp", input_dim=12, output_dim=12, hidden_dim=256,
                num_blocks=4, num_hidden_layers=3, dropout=0.0):
    """Build the model dynamically and print parameter count."""
    if arch == "resmlp":
        model = IKResidualMLP(
            input_dim=input_dim,
